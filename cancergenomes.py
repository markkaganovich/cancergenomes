...
def print_related_genes(gene, outputfilename):
	...
	l = session.query(Mutations).filter(and_(Mutations.c.Hugo_Symbol == gene, Mutations.c.Variant_Classification != 'Silent')).all()
	l_set = list(set(l))
	...
	for s in samples:
		...
		for i in s_mutations:
			out.write(i.cancer + '\t' + str(i.Tumor_Sample_Barcode) +'\t' + str(i.Chromosome) +'\t'+ str(i.Start_Position) + '\t' +str(i.Variant_Classification) + '\t' + str(i.Reference_Allele) + '\t' + str(i.Tumor_Validation_Allele1) + '\t' + str(i.Tumor_Validation_Allele2) + '\t' + str(i.Tumor_Seq_Allele1) + '\t' + str(i.Tumor_Seq_Allele2) +'\n' )
