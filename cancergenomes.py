...
def print_related_genes(gene, outputfilename):
	...
	for s in samples:
		...
		for i in s_mutations:
			out.write(i.cancer + '\t' + str(i.Tumor_Sample_Barcode) +'\t' + str(i.Chromosome) +'\t'+ str(i.Start_Position) + '\t' +str(i.Variant_Classification) + '\t' + str(i.Reference_Allele) + '\t' + str(i.Matched_Norm_Seq_Allele1) + '\t' + str(i.Matched_Norm_Seq_Allele2) + '\t' + str(i.Tumor_Seq_Allele1) + '\t' + str(i.Tumor_Seq_Allele2) +'\n' )
